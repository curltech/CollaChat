<template lang="pug">
    q-dialog.bg-c-grey-0.text-c-grey-10(v-model="$store.state.videoDialog" id='videoDialog' persistent :maximized = 'ifMobileSize || $store.state.ifMobileStyle || fullSize')
        ...
        q-card.message-dialog-card(:class="dialogSizeClass" v-if="$store.state.currentCallChat && ($store.state.currentCallChat.subjectType === SubjectType.CHAT) && $store.state.currentCallChat.callType === 'video'")
            q-card-section.zoom-video-section.zoom-video-section-ios(@click="zoomVideoChange" v-show = "$store.state.currentCallChat && $store.state.currentCallChat.stream")
                .row
                    .col-3
            ...
            q-toolbar.linkman-video-toolbar.justify-center
                ...
                q-fab(color="primary" push :icon="chatMute ?'volume_off':(audioToggle === 'speaker'?'speaker':'volume_up')" v-if = "!Platform.is.android && $store.state.currentCallChat.stream" direction="right" padding="xs")
        ...
        q-card.message-dialog-card(:class="Platform.is.ios?'ios-linkman-video':'linkman-video'" v-if="$store.state.currentCallChat && $store.state.currentCallChat.subjectType === SubjectType.GROUP_CHAT")
            q-toolbar.group-video-toolbar
                ...
                //q-btn-dropdown.text-primary.chatmute-dropdown(:icon="chatMute?'volume_off':'volume_up'" v-if = "Platform.is.android && $store.state.currentCallChat.stream && $store.state.currentCallChat.stream.length > 1 " style ='width:10vw')
                    q-list
                        ...
                        q-item(clickable v-close-popup @click="changeDropdownChatMute('speaker')")
                            q-item-section
                                q-item-label {{$t('Microphone')}}
                q-fab(color="primary" push :icon="chatMute ?'volume_off':(audioToggle === 'speaker'?'speaker':'volume_up')" v-if = "Platform.is.android && $store.state.currentCallChat.stream && $store.state.currentCallChat.stream.length > 1 " direction="right" padding="xs")
                    q-fab-action(color="primary" @click="changeDropdownChatMute('mute')" icon="volume_off")
                    q-fab-action(color="primary" @click="changeDropdownChatMute('speaker')" icon="speaker")
                    q-fab-action(color="primary" @click="changeDropdownChatMute('earpiece')" icon="volume_up")
                q-btn.text-primary(v-if = "!(ifMobileSize || $store.state.ifMobileStyle) && $store.state.currentCallChat.stream && $store.state.currentCallChat.stream.length > 1" unelevated round :icon="chatMute?'volume_off':'volume_up'" @click="changeChatMute")
                ...
                q-space
                q-btn.text-primary(v-if = "$store.state.currentCallChat.stream && $store.state.currentCallChat.stream.length > 1" unelevated round :icon="chatMic?'mic':'mic_off'" @click="changeChatMic")
            q-card-section.group-video-section
                ...
                q-card
                    q-card-section.row.zoom-video-section.zoom-video-section-ios(v-show = "$store.state.currentCallChat && $store.state.currentCallChat.stream")
                        .col-6
                            q-btn.text-primary(unelevated round icon="cached" @click="iosGroupVideoFocus" v-if="$store.state.currentCallChat.stream")
                        .col-6
                            video(ref='zoomVideo' autoplay='autoplay' style="float:right")
                    q-card-section.current-video-section(v-show ="$store.state.currentCallChat.stream")
                ...
                q-list.row.group-video-list(v-if="Platform.is.ios && $store.state.currentCallChat && $store.state.currentCallChat.stream")
                    template(v-for="(memberPeerId, index) in $store.state.currentCallChat.callMessage.content")
