              ...
              .col-12
                q-carousel(v-model="slide" swipeable animated padding control-text-color="c-grey" style="height: 210px")
                  q-carousel-slide(name="slide1" class="q-pa-md")
                    ...
                    q-btn-group(flat spread stretch)
                      q-btn.btnIcon.btnMessage(flat stack no-caps :label="$t('Voice Call')" icon="call"  :disable="!(!ifSelfChat && activeStatus($store.state.currentChat))"   @click="initiateCallRequest('audio')")
                      q-btn.btnIcon.btnMessage(flat stack no-caps :label="$t('Video Call')" icon="videocam"  :disable="!(!ifSelfChat && activeStatus($store.state.currentChat))"   @click="initiateCallRequest('video')")
