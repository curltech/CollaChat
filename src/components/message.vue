        ...
        .message-editor-wrap(:class="ifMobileSize || $store.state.ifMobileStyle ? 'bg-c-grey-message-editor' : 'bg-c-grey-message-editor-pc'")
          .message-editor-area
            q-toolbar.row(style="height:40px;min-height:40px" v-if="!(ifMobileSize || $store.state.ifMobileStyle) && !messageMultiSelectMode")
              ...
              q-btn.q-mr-sm.btnIcon(round flat icon="videocam" :disable="ifSelfChat" @click="initiateCallRequest('video')")
              q-btn.q-mr-sm.btnIcon(round flat icon="call" :disable="ifSelfChat" @click="initiateCallRequest('audio')")
              ...
              .col-12
                q-carousel(v-model="slide" swipeable animated padding control-text-color="c-grey" style="height: 210px")
                  q-carousel-slide(name="slide1" class="q-pa-md")
                    ...
                    q-btn-group(flat spread stretch)
                      q-btn.btnIcon.btnMessage(flat stack no-caps :label="$t('Voice Call')" icon="call"  :disable="ifSelfChat"   @click="initiateCallRequest('audio')")
                      q-btn.btnIcon.btnMessage(flat stack no-caps :label="$t('Video Call')" icon="videocam"  :disable="ifSelfChat"   @click="initiateCallRequest('video')")
