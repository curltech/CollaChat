  ...
  div.bg-c-grey-0.drawHeightStyle(v-if="$store.state.currentChat")
    q-tab-panels(v-model="subKind" animated transition-prev="slide-right" transition-next="slide-left")
      q-tab-panel(name="default" style="padding:0px 0px")
        ...
        q-item-label(v-if="$store.state.currentChat && $store.state.currentChat.subjectType === SubjectType.CHAT && $store.state.linkmanMap[$store.state.currentChat.subjectId].blackedMe === true") {{$t("You are in your oppoent's blacklist.")}}
        q-item-label(v-if="$store.state.currentChat && $store.state.currentChat.subjectType === SubjectType.CHAT && $store.state.linkmanMap[$store.state.currentChat.subjectId].droppedMe === true") {{$t("You are no longer your opponent's friend.")}}
                ...
                q-carousel(v-model="slide" swipeable animated padding control-text-color="c-grey" style="height: 210px")
                  q-carousel-slide(name="slide1" class="q-pa-md")
                    q-btn-group(flat spread stretch)
                      ...
                      q-btn.text-primary.btnMessage(flat stack no-caps :label="$t('Album')" icon="image" @click="uploadMobileMessageImage")
