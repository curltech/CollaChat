  ...
  div.bg-c-grey-0.drawHeightStyle(v-if="$store.state.currentChat")
    q-tab-panels(v-model="subKind" animated transition-prev="slide-right" transition-next="slide-left")
          ...
          q-infinite-scroll(style="width:100%" @load="load_message" debounce="100" reverse :offset="50")
            ...
            template(v-for="(message,index) in $store.state.currentChat.messages")
              div.chat-message(:class="messageMultiSelectMode?'message_multiselect_checkboxwrap':''")
                ...
                messageContent(v-if='message.messageType === P2pChatMessageType.CHAT_LINKMAN || message.messageType === P2pChatMessageType.CALL_REQUEST' v-bind:message = "message" entry = "message")
        ...
        .message-editor-wrap
          .message-editor-area
            q-toolbar.text-c-grey-10.row(style="height:40px;min-height:40px" v-if="!Platform.is.mobile && !messageMultiSelectMode")
              ...
              q-btn.q-mr-sm.btnIcon(round flat icon="videocam" @click="$store.initiateCallRequest('video')")
              q-btn.q-mr-sm.btnIcon(round flat icon="call" :disable="!(!ifSelfChat && activeStatus($store.state.currentChat))" @click="$store.initiateCallRequest('audio')")
              ...
              .col-12
                q-carousel(v-model="slide" swipeable animated padding control-text-color="c-grey" style="height: 210px")
                  q-carousel-slide(name="slide1" class="q-pa-md")
                    ...
                    q-btn-group(flat spread stretch)
                      q-btn.btnIcon.btnMessage(flat stack no-caps :label="$t('Voice Call')" icon="call"  :disable="!(!ifSelfChat && activeStatus($store.state.currentChat))"   @click="$store.initiateCallRequest('audio')")
                      q-btn.btnIcon.btnMessage(flat stack no-caps :label="$t('Video Call')" icon="videocam"  :disable="!(!ifSelfChat && activeStatus($store.state.currentChat))"   @click="$store.initiateCallRequest('video')")
                      q-btn.btnIcon.btnMessage(flat stack no-caps :label="$t('Collection')" icon="bookmarks" @click="openCollection")
        ...
        q-list
          q-item
            ...
            q-item-section(avatar)
              q-btn.btnIcon(dense flat round size="28px" icon="add" @click="showAddGroupChatAndMember")
      ...
      q-tab-panel(name="GROUP_CHATDetails" style="padding:0px 0px")
        ...
        q-list
          q-item
            ...
            q-item-section(avatar v-for="(groupChatMember, index) in ($store.state.groupChatMap[$store.state.currentChat.subjectId] ? $store.state.groupChatMap[$store.state.currentChat.subjectId].groupMembers : [])" v-if="groupChatMember.memberType === MemberType.OWNER && groupChatMember.memberPeerId !== $store.state.myselfPeerClient.peerId" :key="groupChatMember.memberPeerId") {{$store.state.linkmanMap[groupChatMember.memberPeerId] && $store.state.linkmanMap[groupChatMember.memberPeerId].givenName ? ($store.state.linkmanMap[groupChatMember.memberPeerId].givenName.length > 3 ? $store.state.linkmanMap[groupChatMember.memberPeerId].givenName.substr(0, 3) + '...' : $store.state.linkmanMap[groupChatMember.memberPeerId].givenName) : (groupChatMember.memberAlias ? (groupChatMember.memberAlias.length > 3 ? groupChatMember.memberAlias.substr(0, 3) + '...' : groupChatMember.memberAlias) : ($store.state.linkmanMap[groupChatMember.memberPeerId] && $store.state.linkmanMap[groupChatMember.memberPeerId].name ? ($store.state.linkmanMap[groupChatMember.memberPeerId].name.length > 3 ? $store.state.linkmanMap[groupChatMember.memberPeerId].name.substr(0, 3) + '...' : $store.state.linkmanMap[groupChatMember.memberPeerId].name) : $t('NonContacts')))}}
            ...
            q-item-section(avatar v-if="ifIAmGroupOwner($store.state.currentChat)")
              q-btn.btnIcon(dense flat round size="28px" icon="remove" @click="showSelectGroupChatMember")
      ...
      q-tab-panel(name="searchChatHistory" style="padding:0px 0px")
        ...
        q-infinite-scroll(v-if="$store.messageEntry !== 'search' && !searchText" @load="getChatHistory" debounce="100" reverse :offset="50")
          template(v-for="(message, index) in chatMessageResultList")
            messageContent(v-if='message.messageType === P2pChatMessageType.CHAT_LINKMAN' v-bind:message = "message" entry = "historyMessage")
      ...
      q-tab-panel(name="selectGroupChatMember" style="padding:0px 0px")
        q-toolbar
          q-btn(flat round dense icon="keyboard_arrow_left" @click="selectGroupChatMemberBack")
          q-toolbar-title(align="center") {{ selectGroupChatMemberFlag  === 'removeGroupChatMember' ? $t('Remove Group Member') : $t('Select Group Member') }}
          q-btn(flat round dense icon="check" :disable="selectedGroupChatMembers.length < 1 && !selectedGroupChatMemberPeerId" :label="(selectedGroupChatMembers.length > 0 ? '(' + selectedGroupChatMembers.length + ')' : '')" :class="selectedGroupChatMembers.length > 0 || selectedGroupChatMemberPeerId ? 'text-primary' : 'c-grey-0'" @click="doneSelectGroupChatMember")
        q-toolbar(insert)
    ...
    q-dialog(v-model="$store.state.imageMessageViewDialog")
      q-card
        q-card-section(class="row items-center")
          img#dialog-image(:src='$store.state.imageMessageSrc' style='max-width:100%' v-touch-hold:2000.mouse="mediaHold")
