<template lang="pug">
  div(:class="ifMobileSize || $store.state.ifMobileStyle ? 'bg-c-grey-1 heightMobileStyle' : 'bg-c-grey-1 heightStyle'")
    q-toolbar.header-toolbar(:class="ifMobileSize || $store.state.ifMobileStyle ? 'bg-c-grey-1' : 'bg-c-grey-1'")
      ...
      q-btn.btnIcon(flat round dense icon="add_circle_outline" @click="$store.changeKind('newChannel', 'channel');$store.toggleDrawer(true)")
    ...
    q-card(flat :class="ifMobileSize || $store.state.ifMobileStyle ? 'bg-c-grey-1' : 'bg-c-grey-1'")
      q-list
        div(v-for="(channel, index) in ChannelFilteredList" :key="channel.channelId")
          q-item(:active-class="ifMobileSize || $store.state.ifMobileStyle ? 'bg-c-grey-1' : 'bg-c-grey-2'" class="text-c-grey-10")
            ...
            q-item-section
              q-item-label(lines="1" clickable v-ripple @click="channelSelected(channel, index)" ) {{ channel.name }}
            q-item-section(side)
              q-item-label(lines="1") {{ detailDateFormat(channel.updateDate) }}
</template>
