    ...
    q-tab-panels(v-model="subKind" animated transition-prev="slide-right" transition-next="slide-left")
      ...
      q-tab-panel(:style="heightStyle" name="view" class="q-pa-none")
        ...
        div.scroll.header-mar-top.bg-c-white(id="scroll-target-view" :class="ifMobileSize || $store.state.ifMobileStyle ? 'scrollHeightMobileStyle-editor' : 'scrollHeightStyle'")
          ...
          div(v-if="myCollections.c_meta.current && myCollections.c_meta.current.content && myCollections.c_meta.current.content.substr(0,4)!=='http'" v-html="myCollections.c_meta.current.content")
